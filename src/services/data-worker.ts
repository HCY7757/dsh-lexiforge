...
try {
  if (workerData.kind === 'knowledge') {
    ...
    try { parentPort!.postMessage({ value: db.retrieve(workerData.text) }); }
    finally { db.close(); }
  } else {
    const dict = workerData.dict as Dictionary;
    let text = workerData.text as string;
    for (const [from, to] of Object.entries(dict.replacements)) {
      text = text.split(from).join(to);
      if (text.length > 2 * 1024 * 1024) throw new Error('Replacement output too large');
    }
