...
async function main() {
  ...
  if (command === 'knowledge') {
    ...
    if (!input || !output) throw new Error('knowledge INPUT.csv|txt OUTPUT.db');
  ...
  if (action === 'disclaimer') {
    ...
    for (const s of DISCLAIMER_SECTIONS) console.log(`\n${s.title}\n${s.body}`);
    return;
  }
  if (action === 'list') console.log(JSON.stringify(manager.snapshot(), null, 2));
  else if (action === 'install') {
    if (!values.includes('--accept-risk')) throw new Error('Third-party risk acknowledgement required; run `lexiforge <ROOT> disclaimer` to review, then re-run with --accept-risk');
    await manager.install(resolve(values[0] ?? ''), values[1] ?? '', true);
  }
  else if (action === 'market-install') {
    if (!values.includes('--accept-risk')) throw new Error('Pass --accept-risk after reviewing the third-party repository (run `lexiforge <ROOT> disclaimer` first)');
    ...
    const entry = listing.collection
      ...
      : listing.packs[0];
    if (!entry) throw new Error(`Pick one pack of the repository: ${listing.packs.map(p => `${p.id} (${p.name})`).join(', ')}`);
    const file = await market.downloadPack(`${owner}/${repo}`, entry.file);
    try { await manager.install(file, entry.id, true); console.log(`installed ${entry.id} (${entry.name} v${entry.version})`); }
    finally { await rm(file, { force: true }); }
  } else if (action === 'search') console.log(JSON.stringify(await market.searchPacks(values.join(' ')), null, 2));
  else if (action === 'uninstall') await manager.uninstall(values[0] ?? '');
  else if (['enable', 'disable', 'on', 'off', 'order'].includes(action ?? '')) {
    const state = manager.snapshot();
    if (action === 'enable' || action === 'disable') {
      const pack = state.packs.find(p => p.id === values[0]);
      if (!pack) throw new Error('Unknown package'); pack.enabled = action === 'enable';
    }
    if (action === 'order') {
      if (values.length !== state.packs.length || new Set(values).size !== values.length || values.some(id => !state.packs.some(p => p.id === id))) throw new Error('Order must list every installed ID exactly once');
      for (const p of state.packs) p.priority = values.indexOf(p.id);
    }
  ...
  } else throw new Error('Unknown command; run lexiforge help');
}
