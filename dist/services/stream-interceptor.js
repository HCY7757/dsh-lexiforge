...
export class StreamInterceptor {
    processText;
    timeoutMs;
    constructor(processText, timeoutMs = 500) {
        this.processText = processText;
        this.timeoutMs = timeoutMs;
    }
    async *process(source, signal) {
        const chunks = [];
        let bytes = 0;
        for await (const chunk of source) {
            ...
            if (bytes > 16 * 1024 * 1024)
                throw new Error('LexiForge stream buffer exceeds 16 MB');
            chunks.push(chunk);
        }
    ...
    async flush(chunks, signal) {
        ...
        for (const c of chunks)
            if (c.type === 'block-end' && c.block.type === 'text')
                texts.set(c.index, c.block.text);
        ...
        if (finish?.type !== 'finish' || (finish.reason.kind !== 'stop' && finish.reason.kind !== 'max-tokens') || signal?.aborted)
            return chunks;
        ...
        try {
            const replacements = await Promise.race([
                ...
                new Promise((_, reject) => { timer = setTimeout(() => { controller.abort(); reject(new Error('Processing timeout')); }, this.timeoutMs); }),
            ]);
            const emitted = new Set();
            const output = [];
            for (const c of chunks) {
                if (c.type === 'text-delta' && replacements.has(c.index)) {
                    if (!emitted.has(c.index)) {
                        output.push({ ...c, text: replacements.get(c.index) });
                        emitted.add(c.index);
                    }
                }
                    ...
                    if (!emitted.has(c.index))
                        output.push({ type: 'text-delta', index: c.index, text });
            ...
            dbg('flush', 'fallback-original: ' + String(error));
            return chunks;
        }
        finally {
            clearTimeout(timer);
