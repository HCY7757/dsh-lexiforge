...
export class MarketService {
    ...
    async api(path) {
        ...
        if (!r.ok)
            throw new Error(`GitHub HTTP ${r.status}; remaining=${r.headers.get('x-ratelimit-remaining')}; reset=${r.headers.get('x-ratelimit-reset')}; retry-after=${r.headers.get('retry-after')}`);
        return r.json();
    }
    async branchOf(owner, repo) {
        const metadata = await this.api(`/repos/${owner}/${repo}`);
        return metadata.default_branch;
    }
    async searchPacks(query = '', page = 1) {
        if (!Number.isInteger(page) || page < 1 || page > 50 || query.length > 200)
        ...
        try {
            ...
            if (Date.now() - cache.at < 1800000 && Array.isArray(cache.items))
                return cache.items;
        }
        ...
        const data = await this.api(`/search/repositories?q=${encodeURIComponent(query + ' topic:dsh-langpack')}&sort=stars&per_page=20&page=${page}`);
    ...
    async listRepoPacks(input) {
        ...
        if (!response.ok)
            throw new Error(`langpacks.json fetch failed HTTP ${response.status}`);
        const doc = await response.json();
        if (typeof doc !== 'object' || doc === null || doc.version !== 1 || !Array.isArray(doc.packs) || doc.packs.length === 0 || doc.packs.length > 64)
            throw new Error('Invalid langpacks.json (expects version: 1 and packs: [..])');
        const seen = new Set();
        const packs = doc.packs.map((raw) => {
            const p = raw;
            const id = String(p.id ?? '');
            if (!/^[a-z0-9][a-z0-9_-]{0,79}$/.test(id) || seen.has(id))
                throw new Error(`Invalid or duplicate pack id: ${id}`);
            seen.add(id);
            if (typeof p.name !== 'string' || !p.name.trim() || typeof p.version !== 'string' || !p.version.trim() || typeof p.file !== 'string' || !safeFileName(p.file))
                throw new Error(`Invalid pack entry: ${id}`);
    ...
    async fetchZip(owner, repo, branch, path) {
        const response = await rawFile(owner, repo, branch, path);
        if (!response.ok || !response.body)
            throw new Error(`Download failed HTTP ${response.status}; repository must publish ${path}`);
        let size = 0;
        const chunks = [];
        for await (const chunk of response.body) {
            size += chunk.length;
            if (size > 64 * 1024 * 1024)
                throw new Error('ZIP download exceeds 64 MB');
            chunks.push(chunk);
        }
        await mkdir(this.cacheDir, { recursive: true });
        const out = join(this.cacheDir, `download-${randomUUID()}.zip`);
        try {
            await writeFile(out, Buffer.concat(chunks), { flag: 'wx' });
            return out;
        }
        catch (error) {
    ...
    async downloadPack(input, file = 'langpack.zip') {
