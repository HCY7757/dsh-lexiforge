...
export const independentRequests = new WeakSet();
export function workerTask(data, signal) {
...
async function stageKnowledge(pack, text, signal) {
    return workerTask({ kind: 'knowledge', path: await dataFile(pack.root, 'knowledge.db'), text }, signal);
}
async function stageDictionary(pack, text, signal) {
    const before = text;
    text = await workerTask({ kind: 'dictionary', text, dict: await loadDictionary(pack.root) }, signal);
    dbg('process', { pack: pack.id, stage: 'C', changed: text !== before, outHead: text.slice(0, 40) });
    return text;
}
async function stageRewrite(pack, text, knowledge, llm, original, signal) {
    ...
    const request = {
        ...
        messages: [{ id: randomUUID(), role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: prompt }] }],
    };
    ...
    for await (const chunk of llm.stream(request)) {
        ...
        if (output.length > 2 * 1024 * 1024)
            throw new Error('LLM rewrite too large');
        if (chunk.type === 'finish')
            successful = chunk.reason.kind === 'stop';
    }
    if (!successful || !output.trim())
        throw new Error('Independent LLM request did not complete');
    dbg('process', { pack: pack.id, stage: 'A', knowledgeHits: knowledge.length, outHead: output.slice(0, 40) });
    return output;
}
export async function processPacks(text, packs, llm, original, signal) {
    ...
    for (const pack of packs) {
        ...
        if (manifest.mode === 'C') {
            text = await stageDictionary(pack, text, signal);
            continue;
        }
        if (manifest.mode === 'A') {
            text = await stageRewrite(pack, text, [], llm, original, signal);
            continue;
        }
        if (manifest.mode === 'B') {
        ...
        for (const stage of manifest.pipeline ?? []) {
            ...
            if (stage === 'C')
                ...
                pending = await stageKnowledge(pack, text, signal);
                dbg('process', { pack: pack.id, stage: 'B', hits: pending.length });
            }
            else if (stage === 'A')
                text = await stageRewrite(pack, text, pending, llm, original, signal);
        }
    }
    return text;
}
