...
export function safeEntry(root, name) {
    if (!name || /[\\:\x00-\x1f]/.test(name) || name.startsWith('/'))
        throw new Error('Zip Slip: invalid path');
    const parts = name.replace(/\/$/, '').split('/');
    if (parts.some(p => !p || p === '.' || p === '..' || /[. ]$/.test(p) || /^(con|prn|aux|nul|com[0-9]|lpt[0-9])(?:\.|$)/i.test(p)))
        throw new Error('Zip Slip: unsafe component');
...
export async function safeUnpack(zip, target) {
    ...
    try {
        await new Promise((resolveDone, reject) => {
            yauzl.open(zip, { lazyEntries: true, strictFileNames: true, validateEntrySizes: true }, (error, archive) => {
                ...
                const names = new Set();
                const fail = (err) => { if (!failed) {
                    failed = true;
                    archive.close();
                    reject(err);
                } };
                archive.on('error', fail);
                archive.on('end', () => { if (!failed)
                    resolveDone(); });
                archive.on('entry', (entry) => {
                    void (async () => {
                        ...
                        if (mode && mode !== 0x8000 && mode !== 0x4000)
                            throw new Error('ZIP links and special files forbidden');
                        if (entry.generalPurposeBitFlag & 1)
                            throw new Error('Encrypted ZIP forbidden');
                        if (entry.fileName.endsWith('/'))
                            await mkdir(file, { recursive: true });
                        else {
                            if (!/\.(yaml|yml|json|db|txt)$/i.test(entry.fileName))
                                throw new Error('Forbidden file type');
                            await mkdir(dirname(file), { recursive: true });
                            const input = await new Promise((ok, no) => archive.openReadStream(entry, (e, s) => e || !s ? no(e ?? new Error('Missing ZIP stream')) : ok(s)));
